...
const nextQuestion = () => {
    if (shuffledGigs.length === 0) return endGame();

    // Pop and validate — skip rows with missing or unparseable dates
    const randomGig = shuffledGigs.pop();
    const validDate = parseDate(randomGig?.Date);
    ...
    if (type === 0) {
        ...
        options = [...new Set(data.map(g => g.OfficialVenue))]
            .filter(v => v && v !== currentCorrectAnswer);
        ...
        if (isVenueQ) {
            ...
            currentCorrectAnswer = data
                .filter(g => g.OfficialVenue === randomGig.OfficialVenue).length.toString();
        } else {
            questionText = isBand
                ? `How many unique songs have ${bandName} played?`
                : `How many times have you seen ${randomGig.Band} live?`;
            currentCorrectAnswer = isBand
                ? (window.uniqueSongCount || 0).toString()
                : data.filter(g => g.Band === randomGig.Band).length.toString();
        }
        ...
        if (isBand) {
            ...
            const venueGigs = data
                .filter(g => g.OfficialVenue === randomGig.OfficialVenue)
                .sort((a, b) => parseDate(a.Date) - parseDate(b.Date));

            // Guard: if venueGigs is somehow empty, fall back to type 1
            if (venueGigs.length === 0) return nextQuestion();
            currentCorrectAnswer = parseDate(venueGigs[0].Date).getFullYear().toString();

            options = ["1994","2001","2010","1996","2024"].filter(y => y !== currentCorrectAnswer);
        } else {
            ...
            // Guard: should never be empty if we passed the length < 5 check, but be safe
            if (sorted.length === 0) return nextQuestion();
            currentCorrectAnswer = sorted[0].Band;

            options = [...new Set(data.map(g => g.Band))]
                .filter(b => b && b !== currentCorrectAnswer);
        }
    }
