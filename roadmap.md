...
- Schema: `venues`, `performances`, `journals`, `profiles`, `bands` tables
- `date` columns stored as `text` (DD/MM/YYYY) throughout — preserves Journal Key integrity
- RLS: personal journals private to owner, band journals and reference data publicly readable (unauthenticated visitors can browse band archives)
- Partial unique index on journals: `unique(user_id, journal_key) where user_id is not null` — band rows (null user_id) excluded from constraint
- Google Auth configured
- All venues and performances imported
- All 9 personal journals imported with correct user_id backfilled
- Weezer and NFG journals imported (user_id null, publicly readable)
- Band rows identified by `comments = 'Historical Artist Entry'`
- Profiles table populated (9 personal users — no Type/Subject columns, personal only)
- Bands table created separately (Weezer, New Found Glory) — bands are not auth users

**Key decisions made:**
- `profiles` = personal users only. `bands` = separate table, no auth required.
- Band archives browsable without login — intended as a hook for new users.
- `date` stored as text not Postgres date type — avoids format conversion risk.
- Export drops `id`, `user_id`, `journal_key` — app infrastructure only, not part of canonical CSV schema.

**Export SQL (Phase 3):**
```sql
select date, band, notable_support, venue, price, comments,
       went_with, festival::text as "Festival?", festival_lineups,
       photos, official_venue, year, month, day
from journals
